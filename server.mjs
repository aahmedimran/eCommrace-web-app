...
const stripe = Stripe("sk_test_51M4ix5CGVbi0KyInaoOhygdKa50EKd4CHv1IHHM6NwYkvobM89YJ4gG8MOZkrwW3qirOgCoH9LsOHPUjvsnTx4ni00yA3GJm39")
...
app.post("/login", (req, res) => {
  ...
  userModel.findOne(
    ...
    "email firstName secondName age password gender",
    (err, data) => {
      if (!err) {
...
app.post("/signup", (req, res) => {
  let body = req.body;
  if (!body.firstName || !body.secondName || !body.email || !body.password) {
    res.status(400).send(
...
app.delete("/product/:id", async (req, res) => {
  ...
  try {
    let deleted = await productModel
